...
public class LocalUtils implements ApplicationListener<ApplicationReadyEvent> {
	...
	String[] allLocalFileNames = { "error_english.properties", "error_hindi.properties" };
	...
	private Map<String, String> loadLocalFile(String fileName) throws IOException

	{
		Map<String, String> propertyMap = new HashMap<String, String>();
		Properties engPropFile = new Properties();
		
		InputStream stream = LocalUtils.class.getClassLoader().getResourceAsStream(fileName);
	...
	private Map<String, Map<String, String>> readAllLocalFiles() throws IOException {
		...
		for (String currentLocalFile : allLocalFileNames) {
			...
			else if(currentLocalFile.equalsIgnoreCase("error_hindi.properties")) {
				key = LOCALE_HI;
			}
			else {
				key = LOCALE_EN;
	...
	public static String getStringLocale(String locale, String key)
	{
		...
		{
			case LOCALE_EN:
				returnString = allPropFiles.get(LOCALE_EN).get(key);
			break;
			case LOCALE_HI:
				returnString = allPropFiles.get(LOCALE_HI).get(key);
			break;
			default:
				returnString = allPropFiles.get(LOCALE_EN).get(key);
		}
