...
public class InstamojoPaymentGateway {
	...
	private final static Logger logger = Logger.getLogger(InstamojoPaymentGateway.class);
	...
	static {
		try {
			...
			propFile.load(
					InstamojoPaymentGateway.class.getClassLoader().getResourceAsStream("PaymentGateway.properties"));
	...
	public static JSONObject paymentOrder(PaymentGatewayRequestBean pgRequestBean, Users users) throws APIExceptions {
		...
		// The payement gateway does not work with phone numbers appended with country
		// code, we need to remove it.
		dataValidations.phoneNumberValidation(users.getUserName());
		String phoneNumber = users.getUserName();
		if (phoneNumber.length() == 12)
			phoneNumber = phoneNumber.substring(2, phoneNumber.length());
		order.setPhone(phoneNumber);

		dataValidations.paymentGatewayMinimunAmountValidation(pgRequestBean.getAmountRecieved());
		...
		if (isOrderValid) {
			try {
				...
				jsonObject = new JSONObject(createPaymentOrderResponse.getJsonResponse());

				logger.info("status of the payment order:-" + createPaymentOrderResponse.getPaymentOrder().getStatus());
	...
	public static JSONObject getDetailsByTransactionId(String transactionID) {
		...
		try {
			...
			jsonObject = new JSONObject(paymentOrderDetailsResponse.getJsonResponse());
			logger.info("JSON RESPONSE (By Transaction Number) >>>" + paymentOrderDetailsResponse.getJsonResponse());
			logger.info("status of the payment order by ID >>>>" + paymentOrderDetailsResponse.getStatus());
		} catch (ConnectionException | JSONException e) {
			logger.error(e.toString(), e);
			return null;
		} catch (Exception e) {
